...
const handleDrop = (e) => {
    ...
    if (draggedItem.classList.contains("woodenbox")) {
        const newBox = new Objects({ x: x, y: y, image: boxImage, width: 75, height: 75,classname:"boxclass" });
        boxes.push(newBox);
        newBox.draw();
        ...
        const newTrap2 = new Objects({ x:x, y: 420, image: createimage("media/trap2.png"), width: 75, height: 40 ,classname:"trap2"});
        objects.push(newTrap2);
        obstacles.push(newTrap2);
        newTrap2.draw();
    }
    else if (draggedItem.classList.contains("floor1")) {
        const newfloor = new Objects({ x:x, y: e.clientY-rect.top, image: createimage("media/floor1.png"), width: 175, height: 70,classname:"floor1"});
...
function updateZombies() {
 ...
 for (let i = 0; i < zombies.length; i++) {
    ...
    zombies.forEach(zombie => {
        zombie.update();
        console.log("at 464", zombie.isattacking);
        zombie.draw();
    });
...
zombies.forEach((zombie) => {
  if (collision(player, zombie)) {
    ...
    zombie.velocity.x = 0; 
    console.log("collision at 714", zombie.isattacking);

    if (
      player.position.y + player.height <=
      zombie.position.y + player.velocity.y
    ) {
      player.velocity.y = 0;
      onfloor = true;
      player.position.y = zombie.position.y - player.height;
      if (!immune) {
        player.health -= 0.05;
      }
      if (player.health < 0) {
      ...
      console.log("00000000000000000000", player.position.x);
      let collisionDirection = zombie.facingRight ? 1 : -1;
      if (zombie.facingRight) {
        player.velocity.x = keys.right.pressed ? 2 : 0.5; 
      } else {
        player.velocity.x = keys.left.pressed ? -2 : -0.5; 
      }
      const oppositeZombies = zombies.filter(
        (z) => z !== zombie && collision(player, z)
      );
      let balancedForce = false;

      oppositeZombies.forEach((oppZombie) => {
        let oppCollisionDirection = oppZombie.facingRight ? 1 : -1;
        if (collisionDirection !== oppCollisionDirection) {
          balancedForce = true;
        }
      });
      ...
      if (balancedForce) {
        zombies.forEach((z) => {
          if (collision(player, z)) {
            z.velocity.x = 0;
            if (z.facingRight) {
              z.position.x = player.position.x - z.width;
            } else {
              z.position.x = player.position.x + player.width;
            }
          }
        });
      }
    ...
    if (keys.right.pressed && player.position.x<400)
        ...
        function collision(zombie, obj) {
          return (
            zombie.position.x < obj.position.x + obj.width &&
            zombie.position.x + zombie.width > obj.position.x &&
            zombie.position.y < obj.position.y + obj.height &&
            zombie.position.y + zombie.height > obj.position.y
          );
        }
...
for (let i = 0; i < zombies.length; i++) {
    ...
    for (let j = 0; j < objects.length; j++) {
        ...
        if (collision(zombie, object) && !(object.classname==="floor1")) {
            zombie.health -= 0.1;
            if (zombie.health <= 0) {
                zombie.health = 0;
            }
...
class Player
{
    ...
    {   
        ...
        zombies.forEach((zombie)=>{
            ...
            if(collision(player,zombie))
                {zombie.isattacking=true;
                    console.log("collision at 1472", zombie.isattacking);
                    if(!immune){
                    player.health-=0.05;}
                    if(player.health<0)
                        {
...
class Objects
{ 
    constructor({x,y,image,width,height,classname})
    {
        this.position={
            x:x,
            y:y
